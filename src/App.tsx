...
export default function App() {
  ...
  const handleLoad = async (url: string, type: 'sets' | 'cards') => {
    setLoading(true);
    setProgress(0); // Reiniciar barra inmediatamente
    if (progressTimer.current) clearInterval(progressTimer.current);

    // Animación de barra más ágil
    progressTimer.current = setInterval(() => {
      setProgress((prev) => {
        if (prev < 70) return prev + 2; // Sube rápido al principio
        if (prev < 90) return prev + 0.5; // Frena un poco
        if (prev < 98) return prev + 0.1; // Se queda casi al final esperando
        return 98;
      });
    }, 15);
