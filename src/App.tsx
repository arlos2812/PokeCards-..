...
export default function App() {
  ...
  useEffect(() => {
    handleLoad('https://api.pokemontcg.io/v2/sets?orderBy=-releaseDate', 'sets');
  }, []);
  ...
  const loadCards = (set: any) => {
    ...
    setSearchCard('');
    handleLoad(`https://api.pokemontcg.io/v2/cards?q=set.id:${set.id}`, 'cards');
  };
    ...
    <div className="app">
        ...
        <div className="header-container">
          ...
          <div className="header-right">
            <input 
              ...
              onChange={(e) => view === 'sets' ? setSearchSet(e.target.value) : setSearchCard(e.target.value)} 
            />
          </div>
        </div>
      </header>
      ...
      {view === 'cards' && !loading && (
        <div className="filter-section">
          <div className="set-indicator"><h2 className="subtitle">{selectedSetName}</h2></div>
          <div className="filter-bar">
            {TYPE_DATA.map(t => (
              <button 
                key={t.name} 
                className={`filter-btn ${selectedType === t.name ? 'active' : ''}`} 
                style={{ backgroundColor: selectedType === t.name ? t.color : '#1e293b', borderColor: t.color }}
      ...
      <main>
        {loading ? (
          <div className="loader">
            <div className="ball-wrapper">
              <div className="ball-container">
                <img key={ballIndex} src={ALL_BALLS[ballIndex]} className="single-ball" alt="ball" />
              </div>
            </div>
          ...
          <div className="grid">
            {view === 'sets' ? (
              sets.filter(s => s.name.toLowerCase().includes(searchSet.toLowerCase())).map((s: any) => (
                <div key={s.id} className="card-set" onClick={() => loadCards(s)}>
                  <div className="set-logo-container">
                    <img src={s.images.logo} className="set-logo" alt="logo" />
                  </div>
              ...
              cards.filter(c => {
                const matchesSearch = c.name.toLowerCase().includes(searchCard.toLowerCase());
                const matchesType = selectedType === 'Todos' || (c.types && c.types.includes(selectedType));
                return matchesSearch && matchesType;
              }).map((c: any) => (
                <div key={c.id} className="card-pokemon" onClick={() => setSelectedCard(c)}>
                  <img src={c.images.small} className="card-img" alt="card" />
                  <p className="poke-name-text">{c.name}</p>
                  <p className="price-neon">€{c.cardmarket?.prices?.averageSellPrice || '0.00'}</p>
                </div>
              ))
            )}
          </div>
      ...
      {selectedCard && (
        <div className="overlay" onClick={() => setSelectedCard(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <button className="close" onClick={() => setSelectedCard(null)}>×</button>
            <div className="modal-content">
              <div className="modal-left">
                <img src={selectedCard.images.large} className="modal-img" alt="card" />
              </div>
              <div className="modal-right">
                <h1 className="modal-title-main">{selectedCard.name}</h1>
                <div className="modal-badges">
                  <span className="badge-type">{selectedCard.types?.[0] || 'Neutral'}</span>
                  <span className="badge-hp">HP {selectedCard.hp || '??'}</span>
                </div>
                <div className="info-grid">
                  <div className="info-item"><span>RAREZA:</span> <p>{selectedCard.rarity || 'Común'}</p></div>
                  <div className="info-item"><span>ARTISTA:</span> <p>{selectedCard.artist || 'Desconocido'}</p></div>
                  <div className="info-item"><span>NÚMERO:</span> <p>{selectedCard.number} / {selectedCard.set.printedTotal}</p></div>
                  <div className="info-item"><span>EXPANSIÓN:</span> <p>{selectedCard.set.name}</p></div>
                </div>
                <div className="price-section-mini">
                  <p className="price-label">PRECIO MEDIO</p>
                  <p className="price-value-mini">€{selectedCard.cardmarket?.prices?.averageSellPrice || '---'}</p>
                </div>
                <div className="button-group-mini">
                  <a href={selectedCard.cardmarket?.url} target="_blank" rel="noreferrer" className="btn-shop-mini cm">CARDMARKET</a>
                  <a href={`https://www.pricecharting.com/search-products?q=${encodeURIComponent(selectedCard.name + ' ' + selectedCard.number)}`} target="_blank" rel="noreferrer" className="btn-shop-mini pc">PRICECHARTING</a>
                </div>
              </div>
            </div>
          </div>
